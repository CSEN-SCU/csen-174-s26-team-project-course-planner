...
- **Session keys for planner flow:** missing gap list, planning result dict, enriched recommendation list, fingerprint string for last enrichment, course-to-time map, last user preference message text, optional multi-line preference field value, mirrored numeric user id plus username for convenience.
...
7. After parse, overwrite the gap list in session using not-satisfied rows as defined in the gap-rows spec.
