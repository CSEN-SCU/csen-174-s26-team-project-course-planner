...
10. On each weekday card and each **Time TBD** card, offer **Remove & replace from gaps**: when activated, the host re-invokes the memory-augmented planning entrypoint with the current plan as ``previous_plan`` and a follow-up preference that names the course to drop, the vacated weekday/time caption when parsed times exist (otherwise a Time-TBD instruction), and asks for a replacement drawn from ``missing_details``; on success the stored planning result and enrichment fingerprint refresh so ratings reload. The host then **programmatically** compares each newly introduced recommended course code against the Find Course Sections map: it reports whether a workbook row exists, maps the code to a gap ``category`` when possible, and—when the removed card had a parsed pattern—whether any workbook meeting pattern for that new code uses the **same weekday column** (Monday–Friday grid) with **overlapping** start/end times on that day; results appear in an expandable table (`utils/replacement_slot_verify.py`).
