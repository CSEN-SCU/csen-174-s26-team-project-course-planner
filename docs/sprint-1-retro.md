...
|---|---|
| **Jiasheng** | Built the full React + Vite frontend and FastAPI backend from scratch: three-panel layout (weekly calendar, AI chat, session sidebar) wired to the planning agents via a REST API. Added real meeting-time rendering on the calendar from the SCU schedule xlsx, a TBD panel for courses with no confirmed time, and auto-selection of the best available section with collision detection. Strengthened hallucination prevention across both the next-term planner and the new 4-year graduation plan agent — constraining recommendations strictly to courses in the published schedule and stripping any invented course codes post-generation. Shipped conversation persistence per session (chat history survives navigation), session delete, follow-up intent detection so the planner treats subsequent messages as refinements of the current plan, and Workday auto-sync via Playwright so the app can pull academic progress data directly without manual export. |
...
| **Joey** | Ported test suite from initial project based on Ismael's prototype, found in the `project` directory, to the final project location in the `project/course_planner` directory. In addition, consolidated tests from a tests and api/tests folder into one tests folder. This corrected a mistake we made during week 5, where we wrote our test suite for the initial project on top of Ismael's prototype, rather than the new project base. This consolidation allowed us to continue using our necessary testing in the final code location. |
